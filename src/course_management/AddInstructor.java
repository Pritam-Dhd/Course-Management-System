...
public class AddInstructor extends JFrame {
	...
	public AddInstructor() {
		...
		JLabel lblAddCourse = new JLabel("Add Instructor");
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if(id!=null&&name!=null&&m1n!=null&&m2n!=null&&m3n!=null&&m4n!=null&&email!=null){
						...
						String que="SELECT email from user where email='"+email+"'AND user='Teacher'";
						ResultSet r=st.executeQuery(que);
						if(r.next()) {
							String data="SELECT instructor_id FROM instructor WHERE instructor_id = '" + id + "'";
				        	ResultSet rs=st.executeQuery(data);
				        	if(rs.next()) {
				                JOptionPane.showMessageDialog(null, "Instructor already exist", "Error", JOptionPane.ERROR_MESSAGE);
				        	}
				        	else {
				        		...
				        		if(count==4) {
									String query1= "INSERT INTO instructor VALUES ('" + id + "','" + name + "','" 
											+ email + "','" + m1n + "','" + m2n + "','" + m3n + "','" + m4n + "')";
							...
							JOptionPane.showMessageDialog(null, "First create an account of Instructor", "Error", JOptionPane.ERROR_MESSAGE);
						}
					}
					else {
...
//		Close the frame
		...
		btnNewButton.setBounds(285, 26, 102, 26);
